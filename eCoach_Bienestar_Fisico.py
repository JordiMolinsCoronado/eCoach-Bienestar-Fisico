...
PROACTIVE_SCHEDULER_HOUR = int(os.getenv("PROACTIVE_SCHEDULER_HOUR", "9"))
PROACTIVE_SCHEDULER_MINUTE = int(os.getenv("PROACTIVE_SCHEDULER_MINUTE", "0"))
...
async def reply_initial_discovery_with_llm(update: Update, user_text: str) -> None:
    ...
    await update.message.reply_text(answer, reply_markup=anonymized_documents_keyboard())
...
async def delayed_upload_summary_notice(
    ...
    try:
        ...
        upload_inline_keyboard = InlineKeyboardMarkup(
            [[InlineKeyboardButton("Analizar documentos de salud", callback_data="analyze_uploaded_documents")]]
        )
...
async def handle_free_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if detect_physical_wellbeing_initial_message(user_text):
...
def main() -> None:
    ...
    app.add_handler(CallbackQueryHandler(handle_upload_anonymized_documents_button, pattern=r"^upload_anonymized_documents$"))
